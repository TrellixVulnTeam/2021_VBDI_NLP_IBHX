...
class SpanBasedQAScorer(scorer.Scorer):
    ...
    def write_predictions(self):
        ...
        for example in tqdm(self._eval_examples):
            ...
            if self._v2:
                if len(prelim_predictions) == 0 and self._config.debug:
                    tokid = sorted(
                        feature[self._name + "_token_to_orig_map"].keys())[0]
            ...
            if not self._v2:
                ...
                arr = []
                for ss in range(10):
                    arr.append(nbest_json[ss]["text"])
                    all_predictions[example_id] = arr
            else:
        ...
        import pickle
        with open('n_best.pkl', 'wb') as f:
            pickle.dump(all_nbest_json, f)
