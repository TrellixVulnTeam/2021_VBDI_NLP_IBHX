...
def normalize_answer(s):
    text = neologdn.normalize(remove_punc(s))
    return text
...
#   pred_toks = get_tokens(a_pred)
  gold_toks = split_unicode_chrs(normalize_answer(a_gold))
  pred_toks = split_unicode_chrs(normalize_answer(a_pred))
...
#############################################
def format_pred_json(data_pred):
    from collections import OrderedDict
    data_pred = dict(eval(data_pred)) # Data is n_best_prediction NOT the best ones
    best_pred = {}
    for k,v in data_pred.items(): # Get ans with the highest prob
        prob = []
        ...
        for p in article['paragraphs']:
            for qa in p['qas']:
                ...
                if qid not in preds:
                    ...
                    continue
                a_pred = preds[qid]
                # Take max over all gold 
                try:
                    exact_scores[qid] = max(compute_exact(a, a_pred) for a in gold_answers)
                    f1_scores[qid] = max(compute_f1(a, a_pred) for a in gold_answers)
                except:
                    print('Cannot calculate score for {}'.format(qid))
