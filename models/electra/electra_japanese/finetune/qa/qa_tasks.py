...
class QATask(task.Task):
    ...
    def get_prediction_module(self, bert_model, features, is_training,
        ...
        start_top_index = tf.zeros(
            [batch_size, self.config.beam_size], tf.int32)
        ...
        if self.config.answerable_classifier:
            ...
            answerable_loss = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.cast(
                    features[self.name + "_is_impossible"], tf.float32),
        ...
        with tf.io.gfile.GFile(os.path.join(
            ...
            for i, line in enumerate(f):
                ...
                if "header" in paragraph:
                    continue
                self._add_examples(
                    examples, example_failures, paragraph, split)
